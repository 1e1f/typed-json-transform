...
const testASelectors = [["mac", "x64"], ["win"], ["win", "x64"]];
...
describe("should return 1 if", () => {
  ...
  it(`a match !b`, () => {
    assert.equal(select(["a"], "!b"), 1);
  });
  it("a match a || b", () => {
    assert.equal(select(["a"], "a, b"), 1);
  });
  it("[a, b, c] match a || b || !c", () => {
    assert.equal(select(["a", "b", "c"], "a, b, !c"), 1);
  });
});

describe("should return 2 if", () => {
  it("[a] match a && !c", () => {
    assert.equal(select(["a"], "a !c"), 2);
  });

  it("[a, b] match a && b", () => {
    assert.equal(select(["a", "b"], "a b"), 2);
  });

  it("[a, b, c] match (a && b) || c", () => {
    assert.equal(select(["a", "b", "c"], "a b, c"), 2);
  });
});

describe("should return 3 if", () => {
  it("[a, b] match a && b && !c", () => {
    assert.equal(select(["a", "b"], "a b !c"), 3);
  });
});

describe(`should return 0 if`, () => {
  it(`a match b`, () => {
    assert.equal(select(["a"], "b"), 0);
  });
  it(`a match !a`, () => {
    assert.equal(select(["a"], "!a"), 0);
  });
  it("a match a && b", () => {
    assert.equal(select(["a"], "c, a b"), 0);
  });
  it("[a, b] match a && !b", () => {
    assert.equal(select(["a", "b"], "c, a !b"), 0);
  });
  it("[a, b, c] match (a && !b) || (b && !c) || (c && !a)", () => {
    assert.equal(select(["a", "b", "c"], "a !b, b !c, c !a"), 0);
  });
});
